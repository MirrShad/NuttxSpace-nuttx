...
int32_t ETH_MACDMA_Config(void)
{
	int32_t rval;
	...
	uint32_t timeCnt = 0;
	while ((ETH_GetSoftwareResetStatus() == SET) && (timeCnt < 0x04FFFFFF))//等待软件重启网络完成 
	{
		timeCnt++;
	}
	if(timeCnt >= 0x04FFFFFF)
		return -1;
...
//以太网中断服务函数
extern "C"{
void ETH_IRQHandler(int irq, FAR void *context, FAR void *arg)
{
...
{
	int32_t rval=0;
	irq_attach(STM32_IRQ_ETH,ETH_IRQHandler,(void*)0);
...
int initComponents()
{
	...
	{
		Console::Instance()->printf("LAN8720 Init GPIO failed, code %d\r\n", ret);
		return ret;
	}
	ret = InitNetIRQ();
	if(ret <0 )//初始化LAN8720失败
	{
		Console::Instance()->printf("LAN8720 Init IRQ failed, code %d\r\n", ret);
		return ret;
	}			
	ret = lwip_comm_init();
	if(ret <0 )
...
void InitNet()
{
	...
	if(speed&1<<1)syslog("Ethernet Speed:100M\r\n");
	else syslog("Ethernet Speed:10M\r\n");
